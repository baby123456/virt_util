...
int main(){
        ...
        for(i=0;i<4;i++){
            ...
            printf("prr%d init value: %lu\n",Xil_In32(PL_MEMORY_START_ADDR+0x4*i));
        }
        srand((unsigned)time( NULL ) ); // 初始化随机数
        Xil_Out32(PL_MEMORY_START_ADDR+0x4*4,rand());
        printf("taskID base value: %lu\n",Xil_In32(PL_MEMORY_START_ADDR+0x4*4));
        
        Xil_Out32(PL_MEMORY_START_ADDR+0x4*5,rand());
        printf("vmID base value: %lu\n",Xil_In32(PL_MEMORY_START_ADDR+0x4*5));
        printf("\n");
        /*
